...
INSERT INTO cliente (rut, nombre, correo, direccion, telefono, alta) VALUES
('11111111-1', 'Fulanito Feliciano', 'fulanito@gmail.com', 'Calle escondida', '987654322', true);

SELECT nombre, rut FROM cliente 
WHERE rut 
NOT IN (SELECT rut_cliente FROM venta);

SELECT rut_cliente FROM venta;

SELECT c.nombre, c.rut
FROM cliente as c
LEFT JOIN venta as ven
ON c.rut = ven.rut_cliente
WHERE ven.rut_cliente IS NULL;

--2.
--Tabla venta = ven.folio, ven.fecha, ven.monto
--Tabla cliente = c.nombre, c.rut
...
CASE
	WHEN monto BETWEEN 0 AND 1000000 THEN 'Standar'
...
--4. Listar las ventas con la marca de vehiculo más vendido
